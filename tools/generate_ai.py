...
def main():
    ...
    # Music and stingers are generated by tools/generate_music.py: they use a
    # different endpoint and carry a "music" block instead of a "prompt".
    needs = [n for n, d in sounds.items()
             if d.get('source') in ('ai', 'hybrid') and not d.get('music')]
    ...
    is_music = [n for n in targets if sounds[n].get('music')]
    if is_music:
        sys.exit('These are music and belong to tools/generate_music.py: ' + ', '.join(is_music))
